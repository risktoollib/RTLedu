...
You will be using data from the [US Energy Information Administration](https://www.eia.gov/). The API webpage is [EIA Open Data](https://www.eia.gov/opendata/) and you must register to obtain your API key.

You will be doing analysis on [Electricity Data Browser](https://www.eia.gov/electricity/data/browser/).

+ The generation source we care about are as follows when you organize your data:
  + Total Net Generation (TOTAL.ELETPUS.M)
  + From Coal (TOTAL.CLETPUS.M)
  + From Conventional Hydroelectric (TOTAL.HVETPUS.M)
  + From Natural Gas (TOTAL.NGETPUS.M)
  + From Nuclear (TOTAL.NUETPUS.M)
  + From Solar (TOTAL.SOETPUS.M)
  + From Wind (TOTAL.WYETPUS.M)
+ Use `RTL::eia2tidy()` or `RTL::eia2tidy_all()` to extract data from the API. Tickers are provided next to the names above.
