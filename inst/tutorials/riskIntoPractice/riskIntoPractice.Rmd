...
## build data set to github
# EIAkey = "d4670719a51231d01c79e07508243077"
# NG_Exports <- RTL::eia2tidy(ticker = "NG.N9102CN2.M", key = EIAkey, name = "NG_Exports")
# Oil_Exports<- RTL::eia2tidy(ticker = "PET.MCRNTUSCA2.M", key = EIAkey, name = "Oil_Exports")
# 
# arrow::write_feather(NG_Exports, "NG_Exports.feather")
# arrow::write_feather(Oil_Exports, "Oil_Exports.feather")
...
ticks <- tibble::tribble(
  ~ ticker, ~ name,
  "NG.N9102CN2.M", "NaturalGas",
  "PET.MCRNTUSCA2.M", "Oil"
) 

dat <- RTL::eia2tidy_all(tickers = ticks,key = "d4670719a51231d01c79e07508243077",long = TRUE) %>% 
  dplyr::filter(date >= as.Date(fromExports))
dat %>%
  dplyr::mutate(value = dplyr::case_when(series == "NaturalGas" ~ value / 100,
                                         TRUE ~ value)) %>%
  plotly::plot_ly(
    x =  ~ date,
    y =  ~ value,
   ...
   plotly::layout(
    title = list(text = "US Imports from Canada", x = 0.05),
    xaxis = list(title = ""),
    yaxis = list(title = "Oil in kbd and NG in Million Cubic Feet / 100")
  )
```
