...
+ Clean, clear and concise rendering of a business document. You may refer to [quarto.org](https://quarto.org/) or any other online document to customize your output. I reserve the right to award a maximum of `r scales::percent(.5)` for any document not properly presented. Some characteristics of a properly presented document:
...
+ Learning to learn: 
  ...
  + The [arrow package](https://arrow.apache.org/docs/r/) with the use of the `arrow::read_feather()` function is a good example of a package that is not covered in class but is a good example of a package that is used in the industry to import data frames.
...
+ [U.S. Net Imports from Canada of Crude Oil, Monthly](https://github.com/risktoollib/data/raw/master/NG_Exports.feather).
+ [U.S. Natural Gas Pipeline Imports From Canada, Monthly](https://github.com/risktoollib/data/raw/master/Oil_Exports.feather).
...
RTL::dflong %>% 
  ...
  plotly::plot_ly(x=~date, y = ~value,type="scatter", mode="lines") %>% 
   plotly::layout(
    title = list(text = "WTI Price - 1st or Front Contract", x = 0.05),
    xaxis = list(title = ""),
...
All bonds are Constant Maturity Treasuries (CMT). You must use the relevant yield to maturities and coupons from [FRED](https://fred.stlouisfed.org/) as of `r bondDate`.
