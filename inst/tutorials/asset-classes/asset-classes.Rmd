...
**This quiz consists in analyzing financial returns across multiple asset classes.**
...
5. Show a chart of rolling correlation between `VNQ` and `SPY` with a rolling window of 20, 252 and 100 business days. You will be required to use `timetk::slidify()`.
6. Compute the Jarque-Bera statistics.
...
```{r, r, echo = F, eval=T}
ret %>% ggplot2::ggplot(aes(x = ret, col = symbol, fill = symbol)) + 
...
ret.wide <- ret %>%  
  ...
  stats::na.omit() 
corrr::correlate(ret.wide %>% dplyr::select(-date), 
                 use = "everything") %>% 
  corrr::shave() %>% 
...
```{r, echo = F, eval=T}
fig.title = "Correlation Instability between VNQ and SPY"
cor20 <- timetk::slidify(.f = ~cor(.x, .y),
                         .period = 20,
                         .align = "right")
cor252 <- timetk::slidify(.f = ~cor(.x, .y),
                         .period = 252,
                         .align = "right")
cor1000 <- timetk::slidify(.f = ~cor(.x, .y),
                         .period = 1000,
                         .align = "right")
...
```{r, echo = F, eval=T}
spy <- ret %>% dplyr::filter(symbol == "SPY") %>% dplyr::ungroup() %>% dplyr::select(ret)
